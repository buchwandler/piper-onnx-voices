...
def catalog() -> dict:
    return build_catalog(
        upstream(), requested_revision="main", resolved_revision="a" * 40
    )
...
def test_alias_and_filters() -> None:
    ...
    assert [voice["id"] for voice in list_voices(data, language="et")] == [
        "et_EE-news-medium"
    ]
...
def test_revision_is_resolved_before_catalog_fetch(
    ...
    assert all(
        revision in artifact["url"]
        for voice in result["voices"].values()
        for artifact in voice["artifacts"].values()
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            lambda data: data["voices"]["et_EE-news-medium"]["language"].pop("code"),
            "language code",
        ),
        (
            lambda data: data["voices"]["et_EE-news-medium"].update(num_speakers=True),
            "num_speakers",
        ),
        (
            lambda data: data["voices"]["es_ES-sharvard-medium"][
                "speaker_id_map"
            ].update(M=1),
            "duplicate numeric",
        ),
        (
            lambda data: data["voices"]["et_EE-news-medium"].update(
                aliases=["es_ES-sharvard-medium"]
            ),
            "collides",
        ),
        (
            lambda data: data["voices"]["et_EE-news-medium"]["artifacts"][
                "model"
            ].update(filename="../x"),
            "path separators",
        ),
        ...
        (
            lambda data: data["voices"]["et_EE-news-medium"]["artifacts"][
                "model"
            ].update(url="https://example.invalid/x"),
            "pinned source path",
        ),
    ],
)
...
def test_schema_accepts_committed_catalog() -> None:
    schema = json.loads(
        Path("schemas/voice-catalog.schema.json").read_text(encoding="utf-8")
    )
    committed = json.loads(Path("catalog/voices.json").read_text(encoding="utf-8"))
